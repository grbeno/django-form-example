...
def stainer(colors,path):
        ...
        for j in range(height):
            for(index,start,end) in st_color_ints:
                if start <= gray[i,j] and gray[i,j] <= end:
                    ...
                    if index not in color_indexes and index > 0: # no duplicate, no background!
    ...
    percent = [ float(format(data/(size-st_data[0])*100,'.2f')) for data in new_data ]
    
    " Pie diagram " # on Heroku ???
    
    # Sorting color indexes
    sorted_colors = []
    for i in sorted(color_indexes):
        sorted_colors.append(st_colors[i])    
    colors = [ tuple([i/255 for i in rgb]) for rgb in sorted_colors ]
    
    plt.clf()
    plt.pie(percent,colors=colors,labels=percent)
